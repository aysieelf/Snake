...
from src.ui.ui_components import create_rectangle, create_title, get_start_button_rect, _draw_start_button, \
    draw_instructions
from src.ui.ui_components import draw_start_screen
...
class UiComponentsShould(unittest.TestCase):
    ...
    def test_createRectangle_drawsRectangleWithCorrectParameters(self):
        ...
        with patch("pygame.draw.rect") as mock_draw_rect:
            create_rectangle(self.screen, test_color, test_x, test_y, test_width, test_height)

            mock_draw_rect.assert_called_once_with(
                self.screen,
                test_color,
                (test_x, test_y, test_width, test_height)
            )

    def test_drawStartScreen_drawsStartScreen(self):
        with (patch.object(self.screen, "fill"),
    ...
    def test_createTitle_drawsTitle(self):
        with (patch("pygame.font.SysFont") as mock_font_class,
            ...
            mock_font.render.assert_called_once_with(
                "SNAKE", True, (105, 105, 105)
            )
            mock_title_surface.get_rect.assert_called_once_with(
                centerx=200, y=80
            )
    ...
    def test_drawStartButton_drawsStartButton_whenNotHovering(self):
        with (patch("src.ui.ui_components.get_start_button_rect") as mock_get_start_button_rect,
