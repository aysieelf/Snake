...
class GameLoopShould(unittest.TestCase):
    ...
    def test_init_createsEventHandlerAndRenderer(self):
        ...
        with (patch('src.core.game_loop.EventHandler', return_value=mock_event_handler) as mock_eh_class,
    ...
    def test_gameLoop_callsRequiredMethodsInOrder(self):
        ...
        with (patch('src.core.game_loop.EventHandler', return_value=mock_event_handler),
            ...
            self.assertEqual(self.clock.method_calls, [expected_calls[0], expected_calls[0]])
            self.assertEqual(self.game_state.method_calls, [expected_calls[1],
                                                            expected_calls[2],
                                                            expected_calls[1],
                                                            expected_calls[2]])
            self.assertEqual(self.food_system.method_calls, [expected_calls[3], expected_calls[3]])
